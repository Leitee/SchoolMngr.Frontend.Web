...
export class AuthenticationService {
    ...
    logout() {
        ...
        this.currentUserSubject.complete()//TODO: check if it works
    ...
    private getUserFromStoredToken(tokenStr: string | null): User | null {
        let userStr: string; 
        let roleStr: string;
        let user: User;

        if (tokenStr !== null) {
            userStr = jwtDecode<Token>(tokenStr).userdata.toLowerCase();
            user = JSON.parse(userStr);;
            roleStr = jwtDecode<Token>(tokenStr).userrole.toLowerCase();
